const nativeDatamapper = require("./datamapper");
const datamapper = require("../models/datamapper");

const cardController = {
    async getAllCards(req, res) {
        try {
            const cards = await nativeDatamapper.getAllCards();
            // console.log(cards);
            res.json({cards});
        } catch (error) {
    ...
    async getAllCardsStatuses(req, res) {
        ...
        try {
            const statuses = await nativeDatamapper.getAllCardsStatuses(explorerId);
            res.json({statuses});
        } catch (error) {
            res.status(500).send(error);
        }
