...
const cardController = {
    ...
    async markChapter(req, res) {
      ...
      if (!Number.isInteger(explorerId) || !Number.isInteger(chapterId)) {
        return res.status(400).json({ error: 'Invalid explorerId/chapterId' });
      }
      if (status !== 'owned' && status !== 'duplicated') {
        return res.status(400).json({ error: 'Invalid status (must be "owned" or "duplicated")' });
      }
  
      try {
        if (status === 'owned') {
          await datamapper.markChapterOwned({ explorerId, chapterId });
        } else {
